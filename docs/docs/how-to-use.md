...
```bash
uv run --with 'vllm==0.29.0' python main.py --config config/your_own_configuration.yaml
```
...
| `quantization` | ❌ | Enables vLLM quantization for the model, for example `bitsandbytes`. Fast-MIA supports the same quantization methods as vLLM; see the [vLLM Quantization documentation](https://docs.vllm.ai/en/v0.29.0/features/quantization/) for the currently supported methods and hardware requirements. |
| Other keys | ❌ | Forwarded directly to `vllm.LLM`. Select params to fit the model onto your hardware, following the [vLLM.LLM API Reference](https://docs.vllm.ai/en/v0.29.0/api/vllm/index.html#vllm.LLM). |
...
In vLLM 0.29.0, BitsAndBytes requires the official
[vllm-bnb-plugin](https://github.com/vllm-project/vllm-bnb-plugin).
Include it in the same `uv run` environment as vLLM; installing only the
`bitsandbytes` library does not register the quantization method:

```bash
uv run --with 'vllm==0.29.0' --with 'vllm-bnb-plugin==0.0.3' python main.py --config config/llama30b-exp-ref.yaml
```

The plugin automatically registers the quantization method and model loader.
Keep `quantization: "bitsandbytes"` for both models. If `VLLM_PLUGINS` is set,
its comma-separated allowlist must include `bitsandbytes` (or unset the variable
to enable automatic plugin discovery).
...
Values are passed to `vllm.lora.request.LoRARequest`. Select params following the [vLLM.lora.request.LoRARequest API Reference](https://docs.vllm.ai/en/v0.29.0/api/vllm/lora/request.html#vllm.lora.request.LoRARequest).
...
| `aeca` | https://github.com/kecy03/AECA | `lambda_coef` (weight of the NLL volatility term subtracted from the spectral volatility, default `1.0`), `alpha` (Laplace smoothing for the reference token distribution, must be positive, default `1.0`), `file_num` (number of C4 text files, default `15`), `max_token_length` (max token length, default `1024`), `default_i_ref` (self-information assigned to token IDs outside the reference vocabulary, default `0.5`). Shares the C4 token count cache with `dcpdd` when `file_num` and `max_token_length` match. Scores are always computed at temperature `1.0`: the paper's `1.5` needs full-vocabulary logits, which vLLM's prompt logprobs do not expose, so tune `lambda_coef` rather than `sampling_parameters.temperature`. |
