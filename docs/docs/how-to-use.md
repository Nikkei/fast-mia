...
| `aeca` | https://github.com/kecy03/AECA | `lambda_coef` (weight of the NLL volatility term subtracted from the spectral volatility, default `1.0`), `alpha` (Laplace smoothing for the reference token distribution, must be positive, default `1.0`), `file_num` (number of C4 text files, default `15`), `max_token_length` (max token length, default `1024`), `default_i_ref` (self-information assigned to token IDs outside the reference vocabulary, default `0.5`). Shares the C4 token count cache with `dcpdd` when `file_num` and `max_token_length` match. |
