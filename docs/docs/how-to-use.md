...
| `ref` | Uses the difference in loss between the target model and a reference model. Place it **last** in the `methods` list: releasing the reference model destroys the global vLLM parallel state, which can break inference for methods that run after it. | `reference_model` (required. Model configuration for the reference model, same fields as the `model` block). |
