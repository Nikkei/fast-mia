...
def upadate_freq_dist(examples: list[str], tokenizer: AnyTokenizer, freq_dist: list[int], max_token_length: int) -> list[int]:
    example_texts = [example["text"] for example in examples]
    logging.info('encoding start')
    example_input_ids = tokenizer.batch_encode_plus(example_texts, truncation=True, max_length=max_token_length)["input_ids"]
...
class DCPDDMethod(BaseMethod):
    ...
    def process_output(self, output: RequestOutput, input_ids: list[int], freq_dist: list[int]) -> float:
    ...
    def run(
        ...
        """
        cache_dir = ".fastmia_cache"
        os.makedirs(cache_dir, exist_ok=True)
        cache_path = os.path.join(cache_dir, f"freq_dist_{self.file_num}.json")
        ...
        if os.path.exists(cache_path):
            ...
            for i in range(self.file_num):
                fname = f"c4-train.{i:05d}-of-01024.json.gz"
                with gzip.open(os.path.join("data", fname), "rt", encoding="utf-8") as f:
                    examples = []
                    for example in f:
                        example = json.loads(example)
                        examples.append(example)
                    freq_dist = upadate_freq_dist(examples, tokenizer, freq_dist, self.max_token_length)
        ...
        scores = [self.process_output(output, tokenizer.encode(text), freq_dist) for output, text in zip(outputs, texts, strict=False)]
